...
// get users data by @params
async function geAtlastData(params) {
  let query = { userID: params }
  try {
    const Resolve = MainCollection
      .findOne(query);
    return Resolve
  } catch {
...
async function atlasData(param) {
  try {
		let Result = await geAtlastData(param);
  	return Result;
	} catch { return console.log("Error!") }
}

// get users data & update data by @params
async function getAtlasUpdate(setChange, userLimit, userPrem ) {
  let userGetter = { userID: setChange }
  let userSetterLimit = { $set: { limit: userLimit } }
  let userSetterPrem = { $set: { isPremium: userPrem } }
	let swicthQuery =  userSetterLimit
  const makeChange = MainCollection.findOneAndUpdate(
    userGetter, swicthQuery
  );
  return makeChange;
};
async function atlasUpdate(user, changeLimit, changePrem) {
  let Changes = await getAtlasUpdate(user, changeLimit, changePrem);
  return Changes;
}
...
`.trim()
			...
			case 'daftar': case 'regist': case 'register': {
				...
				try {
					...
					await atlasData(parseId).then(async result => {
						...
						if (user == parseId) { return reply("Kamu sudah terdaftar sebelumnya!") };
					})
				} catch {
			...
			case 'howtolimit':{
				let txt = `How to GET Limit?\n\n`
						txt += `Untuk sekarang hanya ada 1 cara\n`
						txt += `1. Subscribe Bot via Donate (5k IDR per user, valid 14days)\n`
						txt += `Limit harian akan di set menjadi 100 limit per hari\n\n`
						txt += `WHY di Limit?, karena makin banyak request/pakai Bot jadi Owner memutuskan untuk menjadikan Bot Freemium Services. GRATIS tapi ada yang BAYAR, jika gratis maka akan di limit, jika bayar maka dapetnya plus plus hehe`
				reply(txt);
			} break
			...
			case 'setpremium':{
				...
				if (!isCreator) return reply('Fitur ini hanya khusus untuk Owner sama tercintahh ...')
					(async () => {
						let userQuery = args.split('@')[0]
						let setPrem = true
						await atlasUpdate(userQuery, undefined, setPrem)
							.then(async data => {
								let user = data
...
╭─❒ 「 User Info 」 
├ Register: ${usersData ? `✅\n├ Name : ${usersData.userName}\n├ Age : ${usersData.age} years\n├ Gender : ${usersData.gender}\n├ Hobby : ${usersData.hobby}\n├ Registered on : ${usersData.registeredOn}` : '❌'}
├ Nomor : @${m.sender.split('@')[0]}
├ Limit : ${usersData.limit} limit
├ XP : ${exp}
├ Premium : ${usersData.isPremium ? '✅' : '❌'}
					...
					let parseId = parseInt(user_id);
						await atlasData(parseId).then(async result => {
							...
							if (!user == parseId) { return reply(NotRegistered) } 
								...
								let getChange = await value -1
								await atlasUpdate(getID, getChange).then(async user => {
									let logChanges = user.limit;
									console.log("Limit changes -1:", logChanges);
								});
						})
